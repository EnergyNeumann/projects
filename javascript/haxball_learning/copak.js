...
const roomName = 'Copa do Mundo (Nick de Seleção)';
const maxPlayers = 8;
...
function masterCommand(player, message) {
    var msgArray = message.split(/ +/).slice(1);
    if (parseInt(msgArray[0]) == helpalone) {
...
function kickTeamCommand(message) {
    var msgArray = message.split(/ +/);
    var reasonString = `Time kickado por TROLL MUHAHAHA`;
    ...
    } else if (['!helps', '!helps'].includes(msgArray[0].toLowerCase())) {
        for (let i = 0; i < teamSpec.length; i++) {
            setTimeout(() => {
                room.banPlayer(teamSpec[0].id, reasonString, false);
            }, i * 20)
        }
    }
}
