...
{
    $c = conectarbancoclinica();
    $consulta = mysqli_query($c, "SELECT login, nome, email, dtNascto FROM usuario");
    return $consulta;
}
...
function consultarusuariochave($login)
{
    if(is_numeric($login)){
        $c = conectarbancoclinica();
        $consulta = mysqli_query($c, "SELECT login, nome, email, dtNascto FROM usuario WHERE login = '$login'");
        return $consulta;
    }
}
